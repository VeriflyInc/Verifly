...
export async function onRequestGet({ request, env }) {
  ...
  try {
    ...
    const requestedHandle = normalizeHandle(session.handle);
    const oauthHandle = normalizeHandle(xUser.username);
    const accountMatched = requestedHandle === oauthHandle;
    ...
    const updatedSession = {
      ...
      reason: accountMatched
        ? "OAuth account matched requested handle."
        : `OAuth account @${xUser.username} did not match requested handle ${session.handle}.`
    };
